...
// --- Lớp cho kết nối UDT ----------------------------
class UDTConnection : public IConnection {
private:
    UDTSOCKET udt_socket_ = UDT::INVALID_SOCK; // Socket handle của UDT
    bool is_connected_ = false;

public:
    UDTConnection() {
        // Constructor rỗng hoặc khởi tạo logic cần thiết
    }

    ~UDTConnection() override {
        disconnect(); // Đảm bảo socket được đóng khi object bị hủy
    }

    bool connect(const std::string& ip, int port) override {
        udt_socket_ = UDT::socket(AF_INET, SOCK_STREAM, 0);
        if (udt_socket_ == UDT::INVALID_SOCK) {
            std::cerr << "UDT: Failed to create socket: " << UDT::getlasterror().getErrorMessage() << std::endl;
            return false;
        }
        ...
        if (getaddrinfo(ip.c_str(), port_str.c_str(), &hints, &res) != 0) {
            std::cerr << "UDT: getaddrinfo failed for " << ip << ":" << port << std::endl;
            UDT::close(udt_socket_);
            udt_socket_ = UDT::INVALID_SOCK;
            return false;
        }
        ...
        bool blocking = true;
        UDT::setsockopt(udt_socket_, 0, UDT_RCVSYN, &blocking, sizeof(bool));
        UDT::setsockopt(udt_socket_, 0, UDT_SNDSYN, &blocking, sizeof(bool));

        int connect_ret = UDT::connect(udt_socket_, (sockaddr*)&serv_addr, sizeof(serv_addr));
        if (connect_ret == UDT::ERROR) {
            std::cerr << "UDT: Failed to connect to " << ip << ":" << port << ": " << UDT::getlasterror().getErrorMessage() << std::endl;
            UDT::close(udt_socket_);
            udt_socket_ = UDT::INVALID_SOCK;
            return false;
        }
        ...
        is_connected_ = true;
        std::cout << "UDT: Successfully connected to " << ip << ":" << port << std::endl;
        return true;
    }

    bool send_data(const void* buffer, size_t length) {
        if (!is_connected_ || udt_socket_ == UDT::INVALID_SOCK) {
            std::cerr << "UDT: Not connected. Cannot send data." << std::endl;
            return false;
        }
        ...
        const char* current_buffer_ptr = (const char*)buffer;
        size_t remaining_length = length;

        while (remaining_length > 0) {
            int bytes_sent_this_call = UDT::send(udt_socket_, current_buffer_ptr, remaining_length, 0);

            if (bytes_sent_this_call == UDT::ERROR) {
                std::cerr << "UDT: Failed to send data chunk: " << UDT::getlasterror().getErrorMessage() << std::endl;
                is_connected_ = false;
                return false; // Gửi thất bại
            }

            if (bytes_sent_this_call == 0) {
                // Với blocking=0, trả về 0 khi remaining_length > 0 thường là dấu hiệu lỗi hoặc ngắt kết nối
                std::cerr << "UDT: send returned 0 with " << remaining_length << " bytes remaining. Possible disconnection." << std::endl;
                is_connected_ = false;
                return false; // Gửi thất bại
            }

            // Nếu thành công (bytes_sent_this_call > 0)
            current_buffer_ptr += bytes_sent_this_call;
            remaining_length -= bytes_sent_this_call;

            // Có thể thêm log ở đây để theo dõi partial send
            // std::cout << "UDT: Sent " << bytes_sent_this_call << "/" << (length - remaining_length) << "/" << length << " bytes in this call." << std::endl;
        }

        return true; // Toàn bộ dữ liệu đã được gửi
    }

    void disconnect() override {
        if (is_connected_ && udt_socket_ != UDT::INVALID_SOCK) {
            std::cout << "UDT: Disconnecting..." << std::endl;
            UDT::close(udt_socket_);
            udt_socket_ = UDT::INVALID_SOCK;
            is_connected_ = false;
            std::cout << "UDT: Disconnected." << std::endl;
        } else {
            std::cout << "UDT: Not connected, no need to disconnect." << std::endl;
        }
    }
};
...
std::unique_ptr<IConnection> create_connection(const AppConfig& config) {
    if (config.protocol == "TLS") {
        ...
        // Đảm bảo TlsConnection được định nghĩa và include
        // return std::make_unique<TlsConnection>();
        // Giả định TLS và TCP có sẵn
        return std::make_unique<TlsConnection>(); // Hoặc TlsConnection tùy code bạn
    } else if (config.protocol == "UDT") {
        std::cout << "UDT Connection!!!" << std::endl;
        return std::make_unique<UDTConnection>();
    } else {
...
int main() {
    ...
    if (UDT::startup() == UDT::ERROR) {
         std::cerr << "Failed to startup UDT: " << UDT::getlasterror().getErrorMessage() << std::endl;
         return 1; // Thoát nếu UDT không khởi động được
    }
    std::cout << "UDT Library started." << std::endl;    

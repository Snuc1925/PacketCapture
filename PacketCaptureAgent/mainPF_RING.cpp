...
std::vector<pfring*> global_pfring_handles;
std::mutex global_handles_mutex;

// --- Cấu trúc gói tin ---
struct CapturedPacket {
    // === THAY ĐỔI: Sử dụng pfring_pkthdr thay cho pcap_pkthdr ===
    pfring_pkthdr header; 
    std::vector<u_char> data;

    // === THAY ĐỔI: Constructor nhận pfring_pkthdr ===
    CapturedPacket(const pfring_pkthdr* h, const u_char* d) : header(*h) {
...
void capture_thread_func(pfring* handle, const std::string& if_name, const AppConfig& config, BoundedThreadSafeQueue<PacketBlock>& queue) {
    ...
    while (!capture_interrupted) {
        pfring_pkthdr header; // Header của pfring
        u_char* packet_data = nullptr;

        // === THAY ĐỔI: Sử dụng pfring_recv() thay cho pcap_next_ex() ===
        // Tham số cuối `1` nghĩa là blocking call (đợi đến khi có gói tin)
        int ret = pfring_recv(handle, &packet_data, 0, &header, 1);

        if (ret > 0) { // ret > 0 có nghĩa là đã nhận được 1 gói tin
        ...
        } else if (ret == PF_RING_ERROR_GENERIC) {
            // Đây là cách pfring_breakloop() báo cho pfring_recv() dừng lại
            std::cout << "PF_RING capture loop interrupted by user on " << if_name << "." << std::endl;
            break;
        } else if (ret < 0) {
            // Các lỗi khác
            std::cerr << "Error reading packets on " << if_name << " with PF_RING, error code: " << ret << std::endl;
            capture_interrupted = true; // Dừng toàn bộ chương trình nếu có lỗi nghiêm trọng
            break;
        }
        // ret == 0 (timeout) thường không xảy ra với blocking call, nhưng để đây cho an toàn
...
void signal_handler(int signum) {
    ...
    for (pfring* handle : global_pfring_handles) {
        if (handle) pfring_breakloop(handle); // Dùng hàm của pfring
    }
}
...
int main() {
    ...
    for (size_t i = 0; i < config.interfaces.size(); ++i) {
        ...
        u_int32_t snaplen = 65535;
        u_int32_t flags = PF_RING_PROMISC; // Bật chế độ promiscuous

        pfring* handle = pfring_open(if_name.c_str(), snaplen, flags);
        if (!handle) {
            std::cerr << "Error opening interface " << if_name << " with PF_RING. "
                      << "Please check name and permissions (run as root?)." << std::endl;
            continue;
        }        

        // --- ÁP DỤNG BỘ LỌC BPF ---
        if (!bpf_filter_string.empty()) {
            // pfring đơn giản hơn, không cần compile trước
            if (pfring_set_bpf_filter(handle, (char*)bpf_filter_string.c_str()) != 0) {
                std::cerr << "Error setting BPF filter for " << if_name << ": " << strerror(errno) << std::endl;
                pfring_close(handle);
                continue;
            }
        }    
        ...
        if (pfring_enable_ring(handle) != 0) {
            std::cerr << "Error activating interface " << if_name << " with PF_RING." << std::endl;
            pfring_close(handle);
            continue;
        }

        if (link_type == -1) {
            // === THAY ĐỔI: Lấy link type từ pfring ===
            link_type = pfring_get_link_type(handle);
        }        
        
        std::cout << "Successfully activated interface: " << if_name << " (using PF_RING)" << std::endl;
        
        { 
            std::lock_guard<std::mutex> lock(global_handles_mutex); 
            global_pfring_handles.push_back(handle); 
        }
    ...
    if (global_pfring_handles.empty()) {
    ...
    {
        std::lock_guard<std::mutex> lock(global_handles_mutex);
        for(size_t i = 0; i < global_pfring_handles.size(); ++i) {
            if (pfring_stats(global_pfring_handles[i], &ps) == 0) {
                std::cout << "\nPF_RING stats for " << interface_map[i] << ":\n";
                std::cout << "  Packets received: " << ps.recv << std::endl;
                std::cout << "  Packets dropped: " << ps.drop << std::endl;
            }
            pfring_close(global_pfring_handles[i]);
        }
        global_pfring_handles.clear();
    }    

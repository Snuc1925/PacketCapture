...
void sender_thread_func(int client_socket) {
    ...
    while (still_capturing || !packet_queue.empty()) {
        if (packet_queue.try_pop(block_to_send)) { // Cần thêm hàm try_pop vào queue
            send_buffer.clear();
            long long block_packets = 0;
            long long block_bytes = 0;

            for (const auto& packet : block_to_send) {
                // uint32_t if_id_net = htonl(packet.interface_id);
                ...
                // memcpy(ptr, &if_id_net, sizeof(if_id_net)); ptr += sizeof(if_id_net);
            ...
            // Queue rỗng, kiểm tra xem các luồng capture còn chạy không
            if (active_capture_threads == 0) {
                still_capturing = false;
            } else {
                // Các luồng capture vẫn chạy nhưng queue đang tạm thời rỗng
                // Ngủ một chút để tránh busy-waiting
